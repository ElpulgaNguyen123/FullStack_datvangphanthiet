...
var storage = multer.diskStorage({
    destination: function (req, file, cb) {
        ...
        cb(null, app.directory_locations);
    },
...
var productUploadFile = multer({ storage: storage }).single('location_image');

// get all products
let getAlllocation = async (req, res, next) => {
    try {
        await pool.query('SELECT * FROM locations', function (error, rows, fields) {
            if (error) throw error;
            res.render('admin/products/locations/locations', {
                title: 'Địa điểm',
                locations: rows,
...
let getEditLocation = async (req, res, next) => {
    try {
...
let postEditLocation = (req, res, next) => {
...
let DeleteLocation = async (req, res, next) => {
    try {
        var arrayError = [],
            successArr = [];
